...
allCountys = []
global rowData      # 每个municipal 12个月的总营销额
rowData = []
...
def OrignData():
    ...
    # 计算出每个 municipal 12个月对应的交易次数 format: list [["Amsterdam",101.6,90.1,77.1,69.1]]
    
    data = cursor.execute("select municipal as municipal, count(id) as turnover, yearmonth as yearmonth from sale group by municipal, yearmonth")
...
def DrawIng():
    page = Page(
        page_title = "coding2.municipal"
    )
    page
    page.add(timeline_map())
    page.add(timeline_bar())
    page.render("2.municipal.html")
